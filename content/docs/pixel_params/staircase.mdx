...
地图的尺寸均为 $128\times 128$，默认缩放等级（Zoom Level）是 $0$。我们记缩放等级为 $Z$，那么可以显示的区域大小可以表示为：

$$
AreaSize=128\times 2^Z\times128\times 2^Z
$$

其中每个像素点可以代表的方块数量是

$$
1\space Map Pixel Represents=2^Z
$$
...
目前 LeviLamina 貌似已经实现了投影 Mod （类 Litematica，需要使用 LeviLamina 模组加载器）；也有类似的基于原生接口（Addons）的 [Holoprint](https://github.com/SuperLlama88888/holoprint)。
但目前我估计有这需求的用户应该不多，所以暂时不加入此功能。

如果您实在需要这个功能，请给我提 issue，那么那时我会做的。
